...
class DB:
    ...
    def change_status(self,hash,count,st1,st2):
        c=self.conn.cursor()
...
if info.mode=="init":
	...
	db.change_status(n.hash,None,"submitted","idle")
	...
	for s in  filelist:
		...
		if s1==statusfile:
			...
			count=int(path[-1:])
		        db.change_status(hash,count,"",execstatus)
	...
	if result.success()==1:
		...
		db.change_status(n.hash,None,"calculated","finished")
		...
		db.add_count(n.hash)
		db.change_status(n.hash,None,"new","idle")
